...
{
	menu();
	int input,number=0;char temp[20];
	...
	do
	{
		...
		case init:
			...
			init_function(member, number); break;
		case add:if (number >= 1000)
		{
			printf("容量已达上限\n"); break;
		}; 
		...
		case sort:
			...
			switch (ii)
			{
			case 1:sortage_function(member, number); printf("按年龄从小到大排序成功！\n"); break;
			case 2:sortname_function(member, number); printf("按名字从小到大排序成功！\n"); break;
			default:printf("指令错误排序失败\n");
				break;
			}
			break;
		case display:display_function(member, number); break;
		default:printf("你输入的序号不合法请重新输入\n");
			break;
		}
	} 
     while (input);
